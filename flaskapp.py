...
<v t="leo.20180702145534.1" a="E"><vh>wsgi start/stop</vh>
<v t="leo.20180702142719.1"><vh>@button start wsgi</vh></v>
<v t="leo.20180702142849.1"><vh>@button stop wsgi</vh></v>
</v>
<v t="leo.20180702145555.1" a="E"><vh>http start/stop</vh>
...
<v t="amd.20160717222221.1" a="E"><vh>@path static</vh>
...
<v t="leo.20180801114529.1" a="E"><vh>trash (20180801)</vh>
...
<t tx="amd.20160714232205.1"># 與 file_selector 搭配的取影像檔程式
...
<t tx="amd.20160714232232.1">def unique(items):
...
<t tx="amd.20160714232359.1">def set_css():
...
<t tx="amd.20160714232513.1">def isAdmin():
...
<t tx="amd.20160714232921.1">@app.route('/login')
def login():
    """login routine"""
    head, level, page = parse_content()
    directory = render_menu(head, level, page)
...
<t tx="amd.20160714233131.1">@app.route('/checkLogin', methods=['POST'])
def checkLogin():
...
<t tx="amd.20160714233320.1"># edit all page content
...
<t tx="amd.20160715000858.1">def editorfoot():
    return '''&lt;body&gt;'''


</t>
<t tx="amd.20160715152825.1">@app.route('/')
def index():
    head, level, page = parse_content()
...
<t tx="amd.20160715154211.1">@app.route('/error_log')
...
<t tx="amd.20160715154321.1">@app.route('/logout')
def logout():
...
<t tx="amd.20160715154534.1">@app.route('/savePage', methods=['POST'])
def savePage():
...
<t tx="amd.20160715154701.1">@app.route('/ssavePage', methods=['POST'])
def ssavePage():
...
<t tx="amd.20160715154923.1">@app.route('/fileuploadform', defaults={'edit':1})
...
<t tx="amd.20160715155334.1">@app.route('/fileaxupload', methods=['POST'])
...
<t tx="amd.20160715155703.1">@app.route('/flvplayer')
...
<t tx="amd.20160715155918.1">@app.route('/imageuploadform', defaults={'edit': 1})
...
<t tx="amd.20160715160036.1">@app.route('/imageaxupload', methods=['POST'])
# ajax jquery chunked file upload for flask
def imageaxupload():
    if isAdmin():
        # need to consider if the uploaded filename already existed.
...
<t tx="amd.20160715160325.1"># 配合 Tinymce4 讓使用者透過 html editor 引用所上傳的 files 與 images
...
<t tx="amd.20160715160505.1">@app.route('/download_list', methods=['GET'])
def download_list():
...
<t tx="amd.20160715160642.1">@app.route('/image_list', methods=['GET'])
def image_list():
    if not isAdmin():
        return redirect("/login")
    else:
...
<t tx="amd.20160715160836.1">@app.route('/load_list')
...
<t tx="amd.20160715161018.1">@app.route('/image_delete_file', methods=['POST'])
...
<t tx="amd.20160715161158.1">@app.route('/doDelete', methods=['POST'])
def doDelete():
...
<t tx="amd.20160715161251.1">@app.route('/image_doDelete', methods=['POST'])
def image_doDelete():
    if not isAdmin():
        return redirect("/login")
    # delete files
...
<t tx="amd.20160715161352.1">@app.route('/search_form', defaults={'edit': 1})
...
<t tx="amd.20160715161508.1">@app.route('/doSearch', methods=['POST'])
def doSearch():
...
<t tx="amd.20160715161642.1">@app.route('/edit_config', defaults={'edit': 1})
...
<t tx="amd.20160715161730.1">@app.route('/saveConfig', methods=['POST'])
def saveConfig():
    if not isAdmin():
        return redirect("/login")
...
<t tx="amd.20160715161923.1"># use to check directory variable data
@app.route('/listdir')
def listdir():
...
<t tx="amd.20160715162003.1">@app.route('/sitemap', defaults={'edit': 1})
...
<t tx="amd.20160717235504.1">@app.route('/delete_file', methods=['POST'])
def delete_file():
...
<t tx="amd.20160718212924.1"># downloads 方法主要將位於 downloads 目錄下的檔案送回瀏覽器
...
<t tx="leo.20180630142017.1">@app.route('/generate_pages')
def generate_pages():
    # 必須決定如何處理重複標題頁面的轉檔
    import os
    # 確定程式檔案所在目錄, 在 Windows 有最後的反斜線
...
<t tx="leo.20180630144325.1"># seperate page need heading and edit variables, if edit=1, system will enter edit mode
# single page edit will use ssavePage to save content, it means seperate save page
'''
@app.route('/get_page2')
...
<t tx="leo.20180630203449.1">def sitemap2(head):
...
<t tx="leo.20180630203542.1">def render_menu2(head, level, page, sitemap=0):
...
<t tx="leo.20180630203825.1">def set_css2():
...
<t tx="leo.20180630233136.1">def syntaxhighlight2():
    return '''
&lt;script type="text/javascript" src="./../static/syntaxhighlighter/shCore.js"&gt;&lt;/script&gt;
&lt;script type="text/javascript" src="./../static/syntaxhighlighter/shBrushJScript.js"&gt;&lt;/script&gt;
&lt;script type="text/javascript" src="./../static/syntaxhighlighter/shBrushJava.js"&gt;&lt;/script&gt;
...
CMSimfly 之後將可以在動態或靜態模式下執行運作

2018.08.08 完成以下功能:

1. 在動態與靜態網頁模式下, 允許重複標題頁面
在動態模式採用允許重複頁面並存, 無論編輯或檢視, 都會調出各重複標題頁面
在靜態模式則從數列元素重複標題後加上數字做為區分, 第一序號重複頁面標題加 1, 接著加 2, 依此類推
2. 當使用者未按照 h1, h2, h3 順序建立頁面時, 會從表單與 sitemap 製作時, 
將 h3 自動提升為 h2 解決產生頁面混亂結果</t>
...
<t tx="leo.20180726214539.1">存檔時移除所有 tag of title

這是一個 bug, 目前若 title 有 tag, 則無法透過 get_page 辨識, 因此必須在存檔前移除所有 tags

bug 180726</t>
<t tx="leo.20180731143510.1">在頁面中加入 h4 標題後, 隨後的所有內容會全部被刪除

20180801 bug, 已經使用 bs4 建立新的解讀函式
</t>
<t tx="leo.20180801114529.1"></t>
<t tx="leo.20180801114537.1">@language python
# old parser, will erase content after h4 and so on (BUG)
...
<t tx="leo.20180915084539.1">納入 Reveal.js 與 Pelican Blog</t>
<t tx="leo.20181002215437.1">def parse_content():
...
<t tx="leo.20181002215447.1">def _remove_h123_attrs(soup):
    tag_order = 0
...
<t tx="leo.20181005174347.1">若空白標題為第一標題, 則會補上 First 標題
若空白標題非第一標題, 則刪除

其他 h1, h2, h3 標註中帶有屬性, 一律刪除

若標題中帶有特殊超文件內容, 若非第一標題, 則移除 h1, h2 或 h3 標註, 留下內容
若帶有特殊超文件內容的標題, 為第一標題, 則最前方加上 &lt;h1&gt;First&lt;/h1&gt; 標註</t>
</tnodes>
</leo_file>